...
import type { DecimatorAPIResponse, DecimatorErrorResponse } from "$lib/types/api";
...
const request = async (url: string, justRetried = false) => {
    ...
    const response: Optional<DecimatorAPIResponse> = await fetch(api, {
        method: "POST",
        redirect: "manual",
...
const probeDecimatorTunnel = async (url: string) => {
